...
def detect_ball(model, device, img , name):
    ...
    save_txt=False
    save_images=True
    save_path='data/output/'+name+'.jpg'
    # Set Dataloader
    img0 = img  # BGR
...
def see(img):
    ...
    name ='last.jpg'
    det = detect_ball(model, device, img,name)
    return det
if __name__ == '__main__':
    model, device = load_weight()
    for i in range (4) :
	
        img = cv2.imread('data/samples/'+str(i+1)+'.jpg')
        name = str(i+1)+'.jpg'
    
        result_obj=detect_ball(model, device, img , name)
    #end = time.time()
